...
function animateInfo1(index){
  ...
  setTimeout(function(){ animateDisapearInfo(obj, next_obj); }, 3500);
  setTimeout(function(){ animateInfo2(index) }, 4000);
}
...
function animateInfo2(index){
  ...
  resetElement(obj, 'info');
  animateAppearInfo(obj,obj);
  setTimeout(function(){ animateSplash(index) }, 1000);
}
...
function animateSplash(index){
  ...
  animateFadeIn(img, 0, 120, img);
  setTimeout(function(){ animateInfoBtn(index) }, 700);
}
...
function animateInfoBtn(index){
  ...
  if (index < 3) {
    index++;
    setTimeout(function(){ animateInfo1(index) }, 4000);
  } else {
    return;
  }
}
...
function animateAppearInfo(obj, next_obj){
  ...
  animateRightSlowDown(obj, -100, 0, next_obj);
}

function animateDisapearInfo(obj, next_obj){
  //console.log('animateDisapearInfo', obj, next_obj)
  animateRightAccelerate(obj, 0, 180, next_obj);
}

function animateRightSlowDown(obj, from, to, next_obj, trigger){
  if(from > to){
    if (obj !== next_obj){
   ...
   // console.log('animateRightSlowDown - next step', next_step, from, to, from - to);
    setTimeout(function(){
      animateRightSlowDown(obj, next_step, to, next_obj, trigger);
    }, 25)
  }
}
...
function animateRightAccelerate(obj, from, to, next_obj, trigger){
  if(from > to){
    if (obj !== next_obj){
      obj.style.visibility = 'hidden';
    }
    ...
    //console.log('animateRightAccelerate - next step', from, to, from - to, next_step);
    setTimeout(function(){
      animateRightAccelerate(obj, next_step, to, next_obj, trigger);
    }, 25)
  }
}

function animateLeftSlowDown(obj, from, to, next_obj, trigger){
  if(from < to){
    if (obj !== next_obj){
      obj.style.visibility = 'hidden';
    }
   ...
   // console.log('animateLeftSlowDown - next step', next_step, from, to, from - to);
    setTimeout(function(){
      animateLeftSlowDown(obj, next_step, to, next_obj, trigger);
    }, 25)
  }
}
